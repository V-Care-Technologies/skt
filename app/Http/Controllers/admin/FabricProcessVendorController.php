...
use App\Models\FabricProcessVendor;
use App\Models\FabricProcessVendorFirm;
...
class FabricProcessVendorController extends Controller
{

    public function index()
    {
        $result['vendors']=FabricProcessVendor::where('is_deleted','0')->orderBy('id', 'DESC')->get();

        return view('admin.fabric_process_vendor.vendor',$result);
    }

    public function manage_vendor(Request $request,$id='')
    {
        if($id>0){
            $arr=FabricProcessVendor::where(['id'=>$id])->first(); 
            ...
            $result['getfirms'] = FabricProcessVendorFirm::where('fabric_process_vendor_id', $id)->get();
        ...
        return view('admin.fabric_process_vendor.manage_vendor',$result);
    }
    ...
    public function manage_vendor_process(Request $request)
    {
        ...
        if($request->post('id')>0){
            $model=FabricProcessVendor::find($request->post('id'));
            $model->updated_by = session('ADMIN_ID');
            $model->updated_at=date('Y-m-d H:i:s');
            $msg="Fabric Process Vendor updated";

        }else{
            $model=new FabricProcessVendor();
            $model->created_by = session('ADMIN_ID');
            $model->created_at=date('Y-m-d H:i:s');
            $model->code='SKT-'.date('Ymdhis');
            $msg="Fabric Process Vendor inserted";
        }
        ...
        if ($request->filled('firm_ids')) {
            ...
            foreach ($firmIds as $index => $firmId) {
                $firm = FabricProcessVendorFirm::findOrNew($firmId);
    ...
    public function delete(Request $request){
       $id = $request->post('id');
       $model=FabricProcessVendor::find($id);
       $model->is_deleted='1';
       $model->save();

       FabricProcessVendorFirm::where('yarn_vendor_id',$id)->delete();
    ...
    public function deletefirm(Request $request){
        ...
        FabricProcessVendorFirm::where('id',$id)->delete();
