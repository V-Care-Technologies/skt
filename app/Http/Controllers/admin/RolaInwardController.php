...
class RolaInwardController extends Controller
{

    public function index()
    {
        $result['rola']=DB::table('rola_inward')
        ...
        return view('admin.rola_inward.index',$result);
    }



    public function manage_rolainward(Request $request,$id='')
    {
    
        if($id>0){
            $arr=DB::table('rola_inward')
            ->leftJoin('yarn_vendor', 'rola_inward.yarn_vendor_id', '=', 'yarn_vendor.id')
            ->select(
                'rola_inward.*',
                'yarn_vendor.name',
            )
            ->where(['rola_inward.id'=>$id])->first(); 
        ...
        return view('admin.rola_inward.manage_rola',$result);
    }

  

    public function manage_rolainward_process(Request $request)
    {
        ...
        try {
            ...
            if ($request->post('id') > 0) {
                $model = RolaInward::find($request->post('id'));
                $model->updated_by = session('ADMIN_ID');
                $model->updated_at = date('Y-m-d H:i:s');
                $msg = "Rola updated";
            } else {
                $model = new RolaInward();
