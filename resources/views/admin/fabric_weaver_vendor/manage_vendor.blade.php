...
@section('page_title', 'Fabric Weaver Vendor')
@section('fabric_weaver_system_select', 'active')
@section('fabric_weaver_vendor_select', 'inner_active')
...
<div class="inner-main-content">   
    <div class="main_page_heading">
        <h2><a href="{{ url('admin/fabricweavervendor') }}" class="back_btn"><i class="fa-solid fa-arrow-left"></i></a>
            @if($id != '0'){{"Update"}}@else{{"Add New"}}@endif Fabric Weaver Vendor</h2>
    </div>
        ...
        <form action="" id="save-form" class="validate" method="post" enctype="multipart/form-data" accept-charset="utf-8">
            ...
            <div class="row">
                ...
                <div class="firm_section">
                    ...
                    <div class="row">
                        <?php if($fabric_process_ids){ $ext=explode(',',$fabric_process_ids);  } ?>
                        @foreach($getProcess as $process)
                        <div class="col-lg-2">
                            <input type="checkbox" name="process_ids[]" <?php if($fabric_process_ids){ foreach($ext as $ex){  if($ex==$process->id){ echo 'checked'; } }  } ?> value="{{$process->id}}"/>{{$process->title}}
                        </div>
                        @endforeach                        
                    </div>    
            ...
            <div class="d-flex align-items-center justify-content-center mt-5 submit_row mb-5">
                <a href="{{ url('admin/fabricweavervendor') }}" class="cancel-btn">Cancel</a>
...
$(document).ready(function() {
    ...
    $(document).on("submit","#save-form",function(e){
        ...
        $.ajax({
            url:"{{ url('admin/fabricweavervendor/manage-vendor-process') }}",
            method:"POST",
            ...
            success(response){
                ...
                if(obj.status=="1"){
                        ...
                        window.location.href = "{{ url('admin/fabricweavervendor') }}";
                }
                else {
    ...
    $(document).on('click', '.remove_row_firm', function() {
        ...
        if (id != '') {
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ url('admin/fabricweavervendor/deletefirm') }}",
                        method: "POST",
                        headers: {
