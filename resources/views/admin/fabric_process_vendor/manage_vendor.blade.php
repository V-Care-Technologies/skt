...
<div class="inner-main-content">   
    <div class="main_page_heading">
        <h2><a href="{{ url('admin/fabricprocessvendor') }}" class="back_btn"><i class="fa-solid fa-arrow-left"></i></a>
            @if($id != '0'){{"Update"}}@else{{"Add New"}}@endif Fabric Process Vendor</h2>
    </div>
...
$(document).ready(function() {
    ...
    $(document).on("submit","#save-form",function(e){
        ...
        $.ajax({
            url:"{{ url('admin/fabricprocessvendor/manage-vendor-process') }}",
            method:"POST",
            ...
            success(response){
                ...
                if(obj.status=="1"){
                        ...
                        window.location.href = "{{ url('admin/fabricprocessvendor') }}";
                }
                else {
    ...
    $(document).on('click', '.remove_row_firm', function() {
        ...
        if (id != '') {
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ url('admin/fabricprocessvendor/deletefirm') }}",
                        method: "POST",
                        headers: {
