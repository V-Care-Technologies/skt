...
@section('tasks_select', 'active')
@section('tasks_select', 'inner_active')
...
<div class="inner-main-content">   
    <div class="main_page_heading">
        <h2><a href="{{ url('admin/tasks') }}" class="back_btn"><i class="fa-solid fa-arrow-left"></i></a>
            @if($id != '0'){{"Update"}}@else{{"Add New"}}@endif Tasks</h2>
    </div>
            ...
            <div class="row">
                    ...
                    <div class="input_box">
                        ...
                        <select name="assign_to" id="assign_to" class="form-control yarn_vendorid xx mySelect2" data-validate="required" data-message-required="Select Users" required>
                            ...
                            <?php foreach($users as $user){?>
                                <option value="{{ $user->id }}" {{ $assign_to == $user->id ? 'selected' : '' }} >{{ $user->name }}</option>
                            <?php }?>
                ...
                <div class="col-lg-4">
                    <div class="input_box">
                        ...
                        <input type="date" name="task_date" value="{{ $task_date }}" placeholder="Enter Task Date" >
                    </div>
                </div>          
                <div class="col-lg-4">
                    <div class="input_box">
                        <p>Task Label<span class="text-danger small">*</span></p>
                        <select class="form-control mySelect1" name="labels[]" multiple="multiple">
                            <!-- Loop through designs and mark as selected if they belong to the folder -->
                            @foreach($tasks as $task)
                                <option value="{{ $task }}" @if(in_array($task, explode(',', $labels))) selected @endif>{{ $task }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="input_box">
                        <p>Title<span class="text-danger small">*</span></p>
                        <input type="text" name="title" value="{{ $title }}" placeholder="Enter Title" data-validate="required" required  data-message-required="Please Enter Party Name">
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="input_box">
                        <p>Description<span class="text-danger small">*</span></p>
                ...
                <div class="col-lg-4">
                    <div class="input_box">
                        <p>Priority</p>
                        <select name="priority"  class="mySelect2 col model_select">
...
$(document).ready(function() {
    ...
    $(document).on("submit","#save-form",function(e){
        ...
        $.ajax({
            url:"{{ url('admin/tasks/manage-tasks-process') }}",
            method:"POST",
